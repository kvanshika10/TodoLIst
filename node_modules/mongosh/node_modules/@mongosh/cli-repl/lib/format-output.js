...
function formatDatabases(output, options) {
    const tableEntries = output.map((db) => [clr_1.default(db.name, 'bold', options), pretty_bytes_1.default(db.sizeOnDisk)]);
    return text_table_1.default(tableEntries, { align: ['l', 'r'] });
}
function formatStats(output, options) {
    return Object.keys(output).map((c) => {
        return `${clr_1.default(c, ['bold', 'yellow'], options)}\n` +
...
function formatListCommands(output, options) {
    const tableEntries = Object.keys(output).map((cmd) => {
        ...
        let result = Object.keys(val).filter(k => k !== 'help').reduce((str, k) => {
            ...
            return str;
        }, `${clr_1.default(cmd, ['bold', 'yellow'], options)}: `);
...
function formatError(error, options) {
    let result = '';
    if (error.name)
        result += `\r${clr_1.default(error.name, ['bold', 'red'], options)}: `;
    if (error.message)
    ...
    if (error.errInfo) {
        result += `\n${clr_1.default(i18n_1.default.__('cli-repl.cli-repl.additionalErrorInfo'), ['yellow'], options)}: `;
        result += inspect(error.errInfo, options);
    }
    if (error.result) {
        result += `\n${clr_1.default(i18n_1.default.__('cli-repl.cli-repl.additionalErrorResult'), ['yellow'], options)}: `;
...
function formatHelp(value, options) {
    ...
    let helpMenu = '';
    if (value.help) {
        helpMenu += `\n  ${clr_1.default(`${value.help}:`, ['yellow', 'bold'], options)}\n\n`;
    }
    ...
    if (value.docs) {
        helpMenu += `\n  ${clr_1.default(i18n_1.default.__('cli-repl.args.moreInformation'), 'bold', options)} ` +
            `${clr_1.default(value.docs, ['green', 'bold'], options)}`;
    }
    return helpMenu;
}
