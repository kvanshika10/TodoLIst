...
class MongoshNodeRepl {
    ...
    async initialize(serviceProvider) {
        var _a, _b, _c;
        const internalState = new shell_api_1.ShellInternalState(serviceProvider, this.bus, this.shellCliOptions);
        const shellEvaluator = new shell_evaluator_1.ShellEvaluator(internalState, (value) => value);
        internalState.setEvaluationListener(this);
        await internalState.fetchConnectionInfo();
        let mongodVersion = (_a = internalState.connectionInfo.buildInfo) === null || _a === void 0 ? void 0 : _a.version;
        ...
        const mongoshCompleter = autocomplete_1.default.bind(null, internalState.getAutocompleteParameters());
        repl.completer =
    ...
    async printStartupLog(internalState) {
        if (this.shellCliOptions.nodb || this.shellCliOptions.quiet) {
            return;
        }
        let result;
        try {
            result = await internalState.currentDb.adminCommand({ getLog: 'startupWarnings' });
            if (!result) {
    ...
    async loadExternalCode(code, filename) {
        const { repl } = this.runtimeState();
        return await util_1.promisify(repl.eval.bind(repl))(code, repl.context, filename);
    }
    async onAsyncSigint() {
        var _a;
        ...
        const mongodVersion = (_a = internalState.connectionInfo.buildInfo) === null || _a === void 0 ? void 0 : _a.version;
    ...
    runtimeState() {
        if (this._runtimeState === null) {
            throw new errors_1.MongoshInternalError('Mongosh not started yet');
        }
    ...
    async close() {
        ...
        if (rs) {
            ...
            if (this.output.writableLength > 0) {
                await events_1.once(this.output, 'drain');
            }
        }
    }
